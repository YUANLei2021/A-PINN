...
def test(dx, x_range):
    ...
    u_pred_noise, v_pred_noise, _, _ = pred(net_model, x_test_ts, x_range)

    u_pred_noise = u_pred_noise.data.numpy().flatten()
    v_pred_noise = v_pred_noise.data.numpy().flatten()
    tp_noise = np.loadtxt('./data/training points(noise).txt')

    plt.figure(figsize=(8, 6))
    plt.plot(x_test, u_pred_noise, label='Predictive u(x) of A-PINN', )
    plt.scatter(tp_noise[:, 0], tp_noise[:, 1], s=30, c='k', marker='x', label='Training data of u(x)')
    plt.plot(x_test, v_pred_noise, label='Predictive v(x) of A-PINN')
    plt.scatter(tp_noise[:, 0], tp_noise[:, 2], s=30, c='g', marker='x', label='Training data of v(x)')
