...
def main():
    ...
    nn_layers = [1, 20, 20, 2]  # neural networks layers
...
def train(pde_parameters, train_parameters, nn_parameters):
    ...
    x_e = torch.FloatTensor(np.array(x_range[1]).reshape(-1, 1)).to(device)
    ...
    u_exact = 1.0 + np.sin(x_test)  # - (12/np.pi**3 + 72/np.pi**5 + 12/np.pi**6) * x_test**2
    ...
    while it < max_iter and loss > min_loss:
        def closure():
            ...
            u_e_pred, v_e_pred = pred(net_model, x_e, x_range)
            ...
            f = torch.cos(x_f) - x_f + 1 / 4 * x_f * v_e_pred - u_f_pred_dx
            e_outputs1 = v_f_pred_dx - u_f_pred ** 2 * x_f
            loss_i = loss_func(u_i_pred, torch.zeros_like(x_i)) + loss_func(v_i_pred, torch.zeros_like(x_i))
            loss_f = loss_func(f, torch.zeros_like(x_f))
            loss_o = loss_func(e_outputs1, torch.zeros_like(x_f))
    ...
    torch.save(net_model, './model/A-PINN_VIDE.pkl')
...
def test(pde_parameters, test_parameters):
    ...
    u_exact2 = u_exact ** 2
    d2_sum = abs_e2.sum()
    exact2_sum = u_exact2.sum()
    l2error = np.sqrt(d2_sum / exact2_sum)
    ...
    plt.tight_layout()
    plt.savefig('./png/A-PINN_VIDE-u(x).png')
    plt.show()
