...
def main():
    ...
    nn_layers = [1, 20, 20, 4]  # neural networks layers
...
def train(pde_parameters, train_parameters, nn_parameters):
    ...
    while it < max_iter and loss > min_loss:
        def closure():
            ...
            f_i_dx = torch.autograd.grad(f_i.sum(), x_i, create_graph=True)[0]
            g_i_dx = torch.autograd.grad(g_i.sum(), x_i, create_graph=True)[0]
            ...
            f1 = 1.0 - 1 / 3 * x_f ** 3 - 1 / 2 * g_pred_dx ** 2 + 1 / 2 * v_pred - f_pred_dxx
            f2 = -1.0 + x_f ** 2 - x_f * f_pred + 1 / 4 * w_pred - g_pred_dxx
            e_outputs1 = v_pred_dx - f_pred ** 2 - g_pred ** 2
            e_outputs2 = w_pred_dx - f_pred ** 2 + g_pred ** 2

            loss_i = loss_func(f_i, torch.ones_like(x_i)) + loss_func(f_i_dx, torch.ones_like(x_i) * 2.0) + \
                     loss_func(g_i, torch.ones_like(x_i) * -1.0) + loss_func(g_i_dx, torch.zeros_like(x_i)) + \
                     loss_func(v_i, torch.zeros_like(x_i)) + loss_func(w_i, torch.zeros_like(x_i))
    ...
    torch.save(net_model, './model/A-PINN_VIDE-2D.pkl')
    np.savetxt('./data/A-PINN_VIDE-2D-loss.txt', loss_record)
    ...
    plt.savefig('./png/A-PINN_VIDE-loss.png')
    plt.show()
...
def test(pde_parameters, test_parameters):
    [_, x_range] = pde_parameters
    [dx] = test_parameters
    net_model = torch.load('./model/A-PINN_VIDE-2D.pkl')
    device = 'cpu'
    ...
    plt.savefig('./png/A-PINN_VIDE-2D-u(x).png')
    plt.show()
